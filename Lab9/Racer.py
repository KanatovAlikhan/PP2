...
bg = pygame.image.load("images/back2.png")
SCREEN_WIDTH = 800
SCREEN_HEIGHT = 554
SPEED = 1
SCORE = 0
...
while True:
    ...
    print(SCORE2)
    '''здесь у вас может возникнуть вопрос почему когда игра начинается score монет не 0 .
    Отвечу так что он начинается с 0 но успевает собрать монету и становится больше . Ради этого я запустил
    код который принтит скор2 ,через терминал можете убедиться что тут он все правильно считает и начинает с 0'''
